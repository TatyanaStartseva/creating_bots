...
from startup import stop_processing, directory_path
import os
import requests
import git
import shutil
...
path = "C:\\Users\\User\\PycharmProjects\\pythonProject6\\mega_bot\\web_app\\bots"
path_video ="C:\\Users\\User\\PycharmProjects\\pythonProject6\\mega_bot\\web_app\\video"
...
async def clone_repo(bot_token, data):
    try:
        ...
        with open (f"{clone_path}\\.env","w") as f:
            f.write(f"REMOVE_SENT_CONFIRMATION = {REMOVE_SENT_CONFIRMATION}\n DATABASE_URI ={DATABASE_URI}\n DATABASE_NAME= {DATABASE_NAME}\n DATABASE_COLLECTION= {str(bot_token_str)}\n")
...
async def update_bot(requests):
    try:
        ...
        exist_bot_path = f"{path}\\{bot_token_str}"
        if os.path.exists(exist_bot_path):
            ...
            return web.Response(text=f"Бот с токеном {exist_bot_path} не найден. Добавьте бота.")
    ...
    try:
        ...
        for step in data['steps']:
            if not isinstance(step, dict):
                return web.Response(text="Элементы списка 'steps' должны быть словарями")
            if 'type' not in step:
                return web.Response(text="Каждый элемент в списке 'steps' должен содержать ключ 'type'")
        ...
        if not os.path.exists(exist_bot_path):
            ...
            return web.Response(text=f"Этот бот уже существует {bot_token}, папка должн называться {bot_token_str}")
    except Exception as e:
        print(f"{e}")
        return web.Response(text=f"{e}")
...
async def get_video_path(requests):
    try:
        ...
        return web.json_response({"error": f"Произошла ошибка при загрузке видео: {str(e)}"})
...
async def upload_video(request):
    try:
        ...
        return web.json_response(
            {"uuid": f"{filename}", "message": "Видео успешно скачено. Его название соответвует ключу uuid"})
    except Exception as e:
        return web.json_response({"error": f"Произошла ошибка при загрузке видео: {str(e)}"})
